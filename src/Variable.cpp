...
Value *Variable::getLLVMValue(Node *stem, FunctionImpl *gfunc) {
	...
	if (gepidx != -1) {
		...
		if (func && func->getThisArg()) {
			...
			Value *thisptr = Builder->CreateLoad(PointerType::get(global_context, 0),
				func->getThisArg(), "derefthis");
			...
			if (!stem->getScope() || this->getScope() == stem->getScope()) {
				...
				Value *parentptr = Builder->CreateLoad(PointerType::get(global_context, 0),
					parentAlloc, "derefparent");
			...
			if (stem->isPointerToPointer()) {
				ptr = Builder->CreateLoad(PointerType::get(global_context, 0), ptr, "deref");
			}
