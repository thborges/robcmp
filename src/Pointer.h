...
public:
	...
	Pointer(const char *name, LanguageDataType type, Node *address, Structure *structure, 
		bool isVolatile = false): 
		name(name), type(type), address(address), structure(structure), isVolatile(isVolatile) {
		if (structure->nextBit > LanguageDataTypeBitWidth[type]) {
			const char *emsg = "The sum of field sizes for %s (%d) is larger than its type size (%d).";
			yyerrorcpp(string_format(emsg, name, structure->nextBit, LanguageDataTypeBitWidth[type]), this);
		}
	}
